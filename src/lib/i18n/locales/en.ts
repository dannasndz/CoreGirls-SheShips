const en = {
  // Navbar
  nav: {
    careers: "Careers",
    references: "References",
    community: "Community",
    takeTheQuiz: "Take the Quiz",
    myProfile: "My Profile",
  },

  // Hero Section
  hero: {
    sloganAlt: "More M, more women in STEM.",
    subtitle: "Take our quiz and discover your path in STEM",
    takeTheQuiz: "Take the Quiz",
    explore: "Explore",
    bannerAlt: "Women shaping the world",
  },

  // STEM Section
  stem: {
    heading: "The Future of STEM Needs You",
    subheading: "Women are underrepresented in every STEM field. You can change that.",
    science: {
      word: "Science",
      stat: "Only 33% of researchers worldwide are women.",
      definition:
        "From Nobel laureate Marie Curie to ecologist Sandra Díaz, women in science are rewriting the rules. Your curiosity could lead the next breakthrough.",
    },
    technology: {
      word: "Technology",
      stat: "Just 26% of tech professionals are women.",
      definition:
        "Grace Hopper invented the first compiler, and today Latina engineers lead innovation at top companies worldwide. Your ideas belong here.",
    },
    engineering: {
      word: "Engineering",
      stat: "Less than 15% of engineers globally are women.",
      definition:
        "From bridges to biomedical devices, women engineers solve the world's toughest challenges. Creativity + logic is your superpower.",
    },
    mathematics: {
      word: "Mathematics",
      stat: "Maryam Mirzakhani was the first woman to win the Fields Medal.",
      definition:
        "Math is the foundation of every STEM field and opens every door. From data science to cryptography — let it open yours.",
    },
  },

  // Career Guide Section
  careerGuide: {
    heading: "Your Journey Starts Here",
    subheading: "three simple steps to find your STEM path",
    step1Title: "Answer the Quiz",
    step1Text: "Take our fun, 7-stage quiz designed just for you.",
    step1Highlight:
      "No pressure, no right or wrong answers — just discover what excites you.",
    step2Title: "Discover your Path",
    step2Text: "Get a personalized STEM profile based on your unique answers.",
    step2Highlight:
      "Learn what the career looks like, what you'd be doing, and why you're the perfect match.",
    step3Title: "Meet Your Role Models",
    step3Text: "Connect with real women thriving in STEM.",
    step3Highlight:
      "Read their stories, learn from their journeys, and see yourself in their success.",
  },

  // Role Models Section
  roleModels: {
    heading: "Women Who Inspire",
    subheading: "real stories from real women breaking barriers in STEM",
    discoverMore: "Discover more role models →",
    julieta: {
      field: "Astrophysics · Mexico 🇲🇽",
      about:
        "A legendary Mexican astrophysicist and science communicator at UNAM. She wrote 40 books, won the UNESCO Kalinga Prize, and dedicated her life to making the Universe accessible to everyone.",
    },
    sandra: {
      field: "Ecology · Argentina 🇦🇷",
      about:
        "One of the most cited environmental scientists in the world. She won the Princess of Asturias Award and was named one of Nature's 10 people who mattered in science in 2019.",
    },
    silvia: {
      field: "Astronomy · Mexico 🇲🇽",
      about:
        "The first Latin American woman to preside over the International Astronomical Union. She won the L'Oréal-UNESCO Award for Women in Science for her work on the chemical composition of nebulae.",
    },
  },

  // CTA Section
  cta: {
    readyToFind: "Ready to Find",
    yourPath: "Your Path?",
    description:
      "It only takes a few minutes. No experience needed, no right or wrong answers — just you, your curiosity, and a world of possibilities waiting to be discovered.",
    startQuiz: "Start the Quiz Now",
  },

  // Footer
  footer: {
    coreGirls: "Core Girls",
    teamDescription:
      "We are a team of three passionate women from Mexico who believe every girl deserves to see herself in STEM.",
    projectName: "+Mujeres STEM",
    projectDescription:
      "Our project was born to inspire, inform, and connect girls and women with the world of STEM.",
    sheships: "💜 #SheShips",
    hackathonTitle: "Global Hackathon 8M",
    hackathonSuffix: "— Build. Share. Launch.",
    hackathonDescription:
      "A 48-hour remote hackathon celebrating International Women's Day (March 6–8) where women and multidisciplinary creators come together to build and publish something real.",
    mission: "Our Mission",
    missionText:
      "Dismantling the structural barriers, lack of information, and invisibility of female leaders that perpetuate the gender gap in STEM across Latin America, empowering the next generation of women to confidently claim their space in science and technology.",
    madeWith:
      "Made with 💜 by Core Girls · #SheShips Hackathon 8M · March 2026",
    notJustHackathon:
      "This is not just a hackathon. This is a space to ship.",
  },

  // Pre-Quiz Welcome
  preQuiz: {
    label: "STEM Discovery Quiz",
    welcome: "Welcome!",
    description:
      "You're about to discover which STEM path fits you best. Answer a few fun questions and find out where your superpower lies!",
    fiveMin: "5 min",
    fifteenQuestions: "15 questions",
    noWrongAnswers: "No wrong answers",
    letsGo: "Let's go! →",
    pathAwaits: "Your path awaits",
    whatDiscover: "What will you discover?",
    scienceDesc: "Explore the mysteries of nature and life",
    techDesc: "Build the digital tools of tomorrow",
    engineeringDesc: "Design and create real-world solutions",
    mathDesc: "Unlock patterns that shape our world",
    quote:
      "Every great scientist, engineer, and innovator started exactly where you are now — curious.",
    preparingQuiz: "Preparing your quiz...",
  },

  // Quiz Page
  quiz: {
    back: "Back",
    seeResults: "See Results",
    next: "Next",
    stage: "Stage {number}",
    optionA: "Option A",
    optionB: "Option B",
    analyzingAnswers: "Analyzing your answers...",
    findingPath: "Finding your STEM path...",
    matchingCareer: "Matching your career...",
    onlyMoment: "This will only take a moment",
    optionC: "Option C",
    optionD: "Option D",
  },

  // Quiz Auth Modal
  quizAuth: {
    resultsReady: "Your results are ready!",
    signupPrompt:
      "Create a free account to discover your perfect STEM career match",
    loginPrompt: "Log in to see your perfect STEM career match",
    email: "Email",
    emailPlaceholder: "you@email.com",
    username: "Username",
    usernamePlaceholder: "Pick a username",
    password: "Password",
    passwordPlaceholder: "Create a password",
    oneMoment: "One moment...",
    showResults: "Show my results!",
    loginAndSee: "Log in & see results!",
    alreadyHaveAccount: "Already have an account?",
    dontHaveAccount: "Don't have an account?",
    logIn: "Log in",
    signUp: "Sign up",
  },

  // Quiz Results
  quizResult: {
    noResult: "No result found",
    takeAgain: "Take the quiz again",
    resultsIn: "Your quiz results are in!",
    youCouldBe: "You could be a...",
    trendingCareer: "Trending Career",
    whyMatches: "Why this matches you",
    basedOnAnswers: "Based on your answers, {percentage}% of your choices aligned with {category}.",
    stemProfile: "Your STEM Profile",
    answersAligned: "{percentage}% of your answers aligned with {category}",
    whatToStudy: "What to Study",
    relatedCareers: "Related Careers",
    leadersInField: "Leaders in This Field",
    otherCareers: "Other {category} Careers You Might Like",
    exploreAll: "Explore {category} Careers",
    browseAll: "Browse All Careers",
    retakeQuiz: "Retake Quiz",
    trending: "Trending",
    more: "+{count} more",
    loadingResults: "Loading your results...",
    categoryDescriptions: {
      S: "You're driven by curiosity about how the world works. You love observing, experimenting, and discovering new things. Science is your superpower!",
      T: "You love building, coding, and creating digital experiences. Technology is your playground, and you're ready to shape the future!",
      E: "You're a natural problem-solver who loves designing and building things. Engineering lets you turn ideas into reality!",
      M: "You see patterns everywhere and love working with numbers and logic. Mathematics is the language you use to understand the world!",
    },
  },

  // Community Page
  community: {
    title: "Community",
    description:
      "Connect with other women in tech. Share stories, ask questions, and inspire each other.",
    loginToContinue: "Log in to continue",
    hi: "Hi, ",
    discussionCount:
      "There are {count} discussion(s) in your circles of interest. Ready to learn something new today?",
    createPost: "Create Post",
    loading: "Loading...",
    loadingPosts: "Loading posts...",
    loadingMore: "Loading more posts...",
    endOfFeed: "You've reached the end of the feed.",
    noPosts: "No posts yet. Be the first to share something!",
    noPostsInCategory: "No posts in {category} yet.",
    home: "Home",
    liked: "Liked",
    groups: "Groups",
    events: "Events",
    projects: "Projects",
    newGroup: "New Group",
    likedPosts: "Liked Posts",
    loadingLiked: "Loading liked posts...",
    noLikedPosts: "You haven't liked any posts yet.",
    allGroups: "All Groups",
    noGroups: "No groups yet. Create the first one!",
    member: "member",
    members: "members",
    post: "post",
    posts: "posts",
  },

  // Comments
  comments: {
    show: "Show comments",
    hide: "Hide comments",
    none: "No comments yet.",
    placeholder: "Write a comment...",
    edit: "Edit",
    delete: "Delete",
    save: "Save",
    cancel: "Cancel",
    editPlaceholder: "Edit your comment...",
    confirmDelete: "Are you sure you want to delete this comment?",
    required: "Comment cannot be empty.",
    saveError: "Could not save the comment.",
    deleteError: "Could not delete the comment.",
  },

  // Post actions
  post: {
    options: "Options",
    edit: "Edit",
    delete: "Delete",
    save: "Save",
    cancel: "Cancel",
    confirmDelete: "Are you sure you want to delete this post?",
    titlePlaceholder: "Post title",
    contentPlaceholder: "Write your post...",
    required: "Title and content are required.",
    saveError: "Could not save the post.",
    deleteError: "Could not delete the post.",
  },

  // Time
  time: {
    mAgo: "{n}m ago",
    hAgo: "{n}h ago",
    dAgo: "{n}d ago",
  },

  // Create Post Form
  createPost: {
    title: "Create Post",
    titlePlaceholder: "Post title",
    contentPlaceholder: "What's on your mind?",
    categories: "Categories",
    tags: "Tags",
    tagPlaceholder: "Add a tag and press Enter",
    cancel: "Cancel",
    post: "Post",
    posting: "Posting...",
  },

  // Create Group Modal
  createGroup: {
    title: "Create a Group",
    groupName: "Group name",
    groupNamePlaceholder: "e.g. Web Devs Sisters",
    descriptionOptional: "Description (optional)",
    descriptionPlaceholder: "What's this group about?",
    cancel: "Cancel",
    createGroup: "Create Group",
    creating: "Creating...",
  },

  // Left Sidebar
  sidebar: {
    navigation: "Navigation",
    home: "Home",
    likedPosts: "Liked Posts",
    groups: "Groups",
    events: "Events",
    projects: "Projects",
    myGroups: "My groups",
    noGroups: "No groups yet -- create or join one!",
    logOut: "Log out",
  },

  // Right Sidebar
  rightSidebar: {
    trendingTopics: "Trending Topics",
    noTags: "No tags yet -- add some to your posts!",
    upcomingEvents: "Upcoming Events",
    noUpcomingEvents: "No upcoming events yet.",
    seeAllEvents: "See all events",
  },

  // Events
  events: {
    title: "Events",
    newEvent: "New Event",
    loadingEvents: "Loading events...",
    noEvents: "No events yet. Create the first one!",
    noFilteredEvents: "No events match the selected filters.",
    filterModality: "Modality",
    filterStatus: "Status",
    filterDates: "Date range",
    clearFilters: "Clear filters",
    upcoming: "Upcoming",
    pastEvents: "Past Events",
    inPerson: "In Person",
    remote: "Remote",
    hybrid: "Hybrid",
    cancelled: "Cancelled",
    joinOnline: "Join online",
    moreInfo: "More info",
    attending: "attending",
    attended: "attended",
    organizedBy: "Organized by {name}",
    cancelRSVP: "Cancel RSVP",
    attend: "Attend",
    eventFull: "Event Full",
    shareToForum: "Share to Forum",
    cancelEvent: "Cancel Event",
    reactivateEvent: "Reactivate Event",
    deleteEvent: "Delete Event",
    confirmDelete: "Are you sure you want to delete this event? This action cannot be undone.",
    youOrganize: "You're the organizer",
    editEvent: "Edit Event",
    createEvent: "Create an Event",
    eventTitle: "Event Title *",
    eventTitlePlaceholder: "e.g. Women in AI Workshop",
    description: "Description *",
    descriptionPlaceholder: "What's this event about?",
    modality: "Modality *",
    meetingLinkLabel: "Meeting Link *",
    meetingLinkPlaceholder: "e.g. https://zoom.us/j/...",
    locationLabel: "Location *",
    locationPlaceholder: "e.g. 123 Main St, City",
    dateLabel: "Date *",
    selectDate: "Select a date...",
    timeLabel: "Time *",
    organizerLabel: "Organizer *",
    myself: "Myself",
    custom: "Custom",
    organizerNameLabel: "Organizer name",
    organizerNamePlaceholder: "e.g. +Mujeres STEM Community",
    participantsLimit: "Maximum spots *",
    unlimited: "Unlimited",
    estimatedAttendees: "Estimated number of attendees",
    estimatedCount: "{count} expected",
    optional: "Optional...",
    statusLabel: "Status *",
    draft: "Draft",
    published: "Published",
    saveDraft: "Save draft",
    keepEditing: "Keep editing",
    saveDraftAndExit: "Save draft",
    discard: "Discard",
    cancelConfirmTitle: "Leave without publishing?",
    cancelConfirmText:
      "You can save it as a draft to continue later, or discard your changes.",
    photo: "Photo",
    addPhoto: "Add",
    uploading: "Uploading...",
    errorImage: "The photo could not be uploaded.",
    externalLink: "External Info Link (optional)",
    externalLinkPlaceholder: "e.g. https://eventbrite.com/...",
    cancel: "Cancel",
    saving: "Saving...",
    saveChanges: "Save Changes",
    errorRequired: "Please fill in all required fields.",
    errorDraftEmpty: "Add at least one field to save the draft.",
    errorPastDate: "The event date and time cannot be in the past.",
    errorLocation: "Place is required to publish the event.",
    errorMeetingLink: "Meeting link is required for remote/hybrid events.",
    errorFailed: "Failed to save event",
    errorGeneric: "Something went wrong",
  },

  // Projects
  projects: {
    title: "Projects",
    newProject: "New Project",
    createProject: "Create Project",
    editProject: "Edit Project",
    saveChanges: "Save Changes",
    loadingProjects: "Loading projects...",
    noProjects: "No projects yet. Create the first one!",
    saving: "Saving...",
    uploading: "Uploading...",
    nameLabel: "Project name *",
    namePlaceholder: "e.g. Robotics Mentorship",
    descriptionLabel: "Description *",
    descriptionPlaceholder: "What's this project about?",
    profileLabel: "Type of interested profile *",
    profilePlaceholder: "e.g. Engineering students, 5th semester",
    encargadasLabel: "Leads",
    addEncargada: "Add lead",
    searchUsersPlaceholder: "Search by name or username...",
    noUsersFound: "No results",
    statusLabel: "Status *",
    modalityLabel: "Modality *",
    startDateLabel: "Start date",
    endDateLabel: "End date",
    optionalDate: "Optional...",
    clearDate: "Clear date",
    placeLabel: "Place *",
    placePlaceholder: "e.g. Engineering Faculty, UABC",
    maxSpotsLabel: "Maximum spots",
    unlimited: "Unlimited",
    areaLabel: "STEM Area",
    imagesLabel: "Images",
    addImage: "Add",
    ledBy: "Led by:",
    ledByMany: "Leads:",
    publishedOn: "Published on {date}",
    profileInterested: "Interested profile",
    spots: "{count} spots",
    interested: "Interested",
    expressInterest: "I'm interested",
    interestedOne: "interested",
    interestedMany: "interested",
    requestJoin: "Request to join",
    requestPending: "Request pending",
    requestAccepted: "Request accepted",
    requestRejected: "Request declined",
    requestFull: "Full",
    requestClosed: "Closed",
    requestClosedDeadline: "Deadline passed",
    pendingRequests: "Pending requests",
    accept: "Accept",
    decline: "Decline",
    errorRequired: "Please fill in all required fields.",
    errorCupo: "Maximum spots must be a whole number greater than 0.",
    errorDates: "The end date cannot be before the start date.",
    errorImage: "Images could not be uploaded.",
    errorFailed: "Failed to save project",
    errorGeneric: "Something went wrong",
    estados: {
      abierto: "Open",
      en_progreso: "In progress",
      cerrado: "Closed",
    },
    modalidades: {
      presencial: "In person",
      virtual: "Virtual",
      hibrido: "Hybrid",
    },
  },

  // Explore Careers Page
  exploreCareers: {
    heading: "Who could that be?",
    searchPlaceholder: "Search careers, skills, or fields...",
    topGrowth: "Growth 2026",
    stemField: "STEM Field",
    clearFilter: "Clear filter",
    disclaimer: "* Listed alphabetically, not in order of relevance.",
    noCareers: "No careers found",
    tryDifferent: "Try a different search or filter",
    science: "Science",
    technology: "Technology",
    engineering: "Engineering",
    mathematics: "Mathematics",
    growth: "Growth",
    whatToStudy: "What to Study",
    relatedCareers: "Related Careers",
    womenLeading: "Women Leading This Field",
  },

  // References Page
  references: {
    heading: "They Did It. So Can You.",
    description:
      "Latina women who have shaped STEM across the world. Get inspired by their stories, they prove there's a place for you too.",
    searchPlaceholder: "Search by name, field, or country...",
    pioneers: "Pioneers",
    currentLeaders: "Current Leaders",
    roleModelsFound: "{count} role model(s) found",
    noRoleModels: "No role models found",
    tryDifferent: "Try a different search or filter",
  },

  // Auth Modal
  auth: {
    welcomeBack: "Welcome back!",
    joinSheShips: "Join +Mujeres STEM",
    email: "Email",
    emailPlaceholder: "you@email.com",
    username: "Username",
    usernamePlaceholder: "Enter your username",
    password: "Password",
    passwordPlaceholder: "Enter your password",
    logIn: "Log in",
    signUp: "Sign up",
    loading: "Loading...",
    dontHaveAccount: "Don't have an account?",
    alreadyHaveAccount: "Already have an account?",
    invalidCredentials: "Invalid credentials",
    registrationFailed: "Registration failed",
    somethingWentWrong: "Something went wrong",
  },

  // Profile Page
  profile: {
    pleaseLogIn: "Please log in to view your profile",
    joined: "Joined {date}",
    since: "Since {date}",
    posts: "Posts",
    comments: "Comments",
    likes: "Likes",
    profileGroups: "Groups",
    myGroups: "My Groups",
    noGroupsYet: "No groups yet",
    myEvents: "My Events",
    careerQuiz: "Career Quiz",
    discoverCareer: "Discover your ideal career path!",
    takeTheQuiz: "Take the Quiz",
    taken: "Taken {date}",
    viewResults: "View results →",
    eventsCreated: "Events Created",
    noEventsYet: "No events yet",
    attendingLabel: "Attending",
    notAttending: "Not attending any events",
    logOut: "Log out",
    hey: "Hey, {username}!",
    latestActivity: "Check out your latest activity and keep building your STEM journey.",
    goToCommunity: "Go to Community",
    noPostsYet: "No posts yet",
    startConversation: "Start a conversation",
    editProfile: "Edit profile",
    save: "Save changes",
    saving: "Saving...",
    cancel: "Cancel",
    add: "Add",
    backToProfile: "Back to my profile",
    basicInfo: "Information",
    profileData: "Profile data",
    aboutMe: "About me",
    noDescription: "You haven't added a description yet.",
    descriptionPlaceholder: "Tell us about yourself...",
    interests: "Interests",
    noInterests: "No interests registered",
    interestsPlaceholder: "Type and press Enter",
    certificates: "Certificates",
    noCertificates: "No certificates yet",
    addCertificate: "Add certificate",
    certificateName: "Certificate name",
    uploadCertificate: "Upload certificate",
    uploadPhoto: "Upload photo",
    uploading: "Uploading...",
    delete: "Delete",
    confirmDelete: "Are you sure you want to delete this item?",
    viewFile: "View file",
    practices: "Experiences",
    noPractices: "No experiences registered yet",
    addPractice: "Add experience",
    editPractice: "Edit experience",
    company: "Company / Institution",
    area: "Area",
    startDate: "Start date",
    endDate: "End date",
    endDateHint: "Leave it empty if the experience is ongoing.",
    inProgress: "Present",
    projects: "Projects",
    noProjects: "No projects yet",
    accountStatus: "Account status",
    userType: "User type",
    fullName: "Full name",
    institution: "Institution",
    campus: "Campus",
    career: "Major",
    semester: "Semester",
    sector: "Sector",
    areaStem: "STEM area",
    subjects: "Subjects taught",
    subjectsPlaceholder: "Type and press Enter",
    laborStart: "Work start date",
    occupation: "Occupation",
    entryDateLabel: "Entry date",
    graduationDate: "Graduation date",
    location: "Location",
    clubs: "Clubs",
    clubsPlaceholder: "Type and press Enter",
    selectPlaceholder: "Select an option",
    updated: "Profile updated successfully.",
    updateError: "Could not update the profile.",
    dateInFuture: "The date cannot be in the future.",
    endBeforeStart: "The end date cannot be before the start date.",
    uploadError:
      "Could not upload the file. Check the format (PDF, JPG, PNG) and the 5 MB size limit.",
    publicProfile: "Public profile",
    viewFullProfile: "View full profile",
    notFound: "User not found",
    membersOnly: "Log in to view this profile",
    statuses: {
      PENDING: "Pending",
      VERIFIED: "Verified",
      REJECTED: "Rejected",
    },
    userTypes: {
      ALUMNA: "Student",
      ACADEMICA: "Academic",
      EGRESADA: "Alumna",
    },
    campusNames: {
      ENSENADA: "Ensenada",
      MEXICALI: "Mexicali",
      TECATE: "Tecate",
      TIJUANA: "Tijuana",
      PLAYAS_DE_ROSARITO: "Playas de Rosarito",
      SAN_QUINTIN: "San Quintin",
      SAN_FELIPE: "San Felipe",
    },
    areas: {
      CIENCIA: "Science",
      TECNOLOGIA: "Technology",
      INGENIERIA: "Engineering",
      MATEMATICAS: "Mathematics",
    },
  },

  // Common / Shared
  common: {
    all: "All",
    previous: "Previous",
    nextPage: "Next",
    cancel: "Cancel",
    delete: "Delete",
  },
} as const;

export default en;
